...
class ParserCore(threading.Thread):
        def __init__(self, bus, states):
                ...
                self.buttons = {b'\x00':'A', b'\x01':'B', b'\x02':'X', b'\x03':'Y',         \
                                b'\x04':'LB', b'\x05':'RB', b'\x06':'Back', b'\x07':'Start',        \
                                b'\x08':'Middle', '\t':'LJ/Button', '\n':'RJ/Button'}
                # List of Joy Names
                self.joys = ['LT', 'RT',                                                                        \
        ...
        def run(self):
                ...
                while 1:
                        ...
                        # BUTTON is PRESSED
                        if self.templist[4]==b'\x01' or self.templist[4]==b'\xFF':
                                self.parse_pressed_button()
                                
                        # BUTTON is RELEASED
                        if self.templist[4]==b'\x00':
        ...
        def parse_pressed_button(self):
                ...
                if self.templist[6]==b'\x01' and self.templist[5]==b'\x00': # Letters,
                                                                                                                                  # Start/Back
                        self.states[self.buttons[self.templist[7]]] = 1
                elif self.templist[6]==b'\x02' and self.templist[5]==b'\x80': # D-Pad L/U
                        if self.templist[7]==b'\x06': # Left
                                self.states['Left'] = 1
                        elif self.templist[7]==b'\x07': # Up
                                self.states['Up'] = 1
                elif self.templist[6]==b'\x02' and self.templist[5]==b'\x7F': # D-Pad R/D
                        if self.templist[7]==b'\x06': # Right
                                self.states['Right'] = 1
                        elif self.templist[7]==b'\x07': # Down
                                self.states['Down'] = 1

        def parse_pressed_joy(self):
                # Updates joy states with values 0-255 as ints
                if self.templist[7]==b'\x02' and self.templist[6]==b'\x02': # LT
                        if ord(self.templist[5])>=128:
                                ...
                                self.states['LT'] = val
                elif self.templist[7]==b'\x05' and self.templist[6]==b'\x02': # RT
                        if ord(self.templist[5])>=128:
                                val = ord(self.templist[5]) - 128
                                self.states['RT'] = val
                        elif ord(self.templist[5])<=127:
                                val = ord(self.templist[5]) + 127
                                self.states['RT'] = val
                elif self.templist[7]==b'\x00' and self.templist[6]==b'\x02': # LJ L/R
                ...
                elif self.templist[7]==b'\x01' and self.templist[6]==b'\x02': # LJ U/D
                ...
                elif self.templist[7]==b'\x03' and self.templist[6]==b'\x02': # RJ L/R
                        if ord(self.templist[5])<=127: # Right
                                val = ord(self.templist[5])
                ...
                elif self.templist[7]==b'\x04' and self.templist[6]==b'\x02': # RJ U/D
                        if ord(self.templist[5])<=127: # Down
                                val = -(ord(self.templist[5]))          # Flip to negative
        ...
        def parse_released_button(self):
                ...
                if self.templist[5]==b'\x00' and self.templist[6]==b'\x01': # Letters,
                                                                                                                                  # Start/Stop,
                                                                                                                                  # L/RJ Button
                        self.states[self.buttons[self.templist[7]]] = 0
                elif self.templist[5]==b'\x00' and self.templist[6]==b'\x02': # D-Pad
                        if self.templist[7]==b'\x07': # D-Pad, Up/Down
                                if self.states['Up']==1: # Up
                                        self.states['Up'] = 0
                                elif self.states['Down']==1: # Down
                                        self.states['Down'] = 0
                        elif self.templist[7]==b'\x06': # D-Pad, Left/Right
